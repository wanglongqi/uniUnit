...
# Fix Hz definition: 1 Hz = 1 cycle/s = 2*pi rad/s (not 1 rad/s)
# Pint's default Hz = 1/s, but physically 1 Hz = 1 cycle/s where 1 cycle = 2*pi rad
ureg.define("hertz = cycle / second = Hz")
...
def check_unit_compatibility(
    q1: Union[pint.Quantity, pint.Unit], q2: Union[pint.Quantity, pint.Unit]
) -> bool:
    """
...
def convert_value(
    value: float, from_unit: Union[pint.Unit, str], to_unit: Union[pint.Unit, str]
) -> float:
    """
    ...
    def parse_unit(unit):
        if isinstance(unit, str):
            ...
            if unit_lower in ("s^-1", "1/s", "/s"):
                return ureg("Hz")
            if unit_lower in ("hz", "hertz"):
                return ureg("Hz")
            return ureg(unit)
        return unit
...
class UnitSystem:
    ...
    @classmethod
    def register_preset(
        cls, name: str, units: Dict[str, str], description: str = ""
    ) -> None:
        """
    ...
    def convert_from(
        self, uin: pint.Quantity, source_system: "UnitSystem"
    ) -> pint.Quantity:
        """
...
class uniUnit:
    ...
    def _get_target_unit(self, dims_tuple: tuple) -> pint.Unit:
        ...
        for dim, exp in dims_tuple:
            target_str = self._udict.get(
                dim, DIMENSION_TO_SHORT.get(dim, dim.strip("[]"))
            )
    ...
    def to_unit(
        self, uin: Union[pint.Quantity, List, Tuple, float, int]
...
def create_custom_unit(
    name: str, value: Union[pint.Quantity, float], unit: Optional[pint.Unit] = None
) -> pint.Unit:
    """
...
def to_unit(
    uin: Union[pint.Quantity, float, int], units: Dict[str, str]
) -> Union[pint.Quantity, List]:
    """
