...
class TestNanoscience(unittest.TestCase):
    ...
    def test_nano_science_energy(self):
        ...
        self.assertIn(
            "microgram",
            str(result.units).lower() or "nanometer" in str(result.units).lower(),
        )
...
class TestCustomUnitSystem(unittest.TestCase):
    ...
    def test_custom_system_conversion(self):
        ...
        custom = UnitSystem(
            "Test", {"kilogram": "mg", "meter": "km", "second": "minute"}
        )
...
class TestHertzConversion(unittest.TestCase):
    """Test Hertz and frequency unit conversions."""

    def test_hz_to_rad_per_s(self):
        """Test Hz to rad/s conversion: 1 Hz = 2*pi rad/s."""
        result = convert_value(1, "Hz", "rad/s")
        self.assertAlmostEqual(result, 2 * math.pi, places=9)

    def test_rad_per_s_to_hz(self):
        """Test rad/s to Hz conversion: 2*pi rad/s = 1 Hz."""
        result = convert_value(2 * math.pi, "rad/s", "Hz")
        self.assertAlmostEqual(result, 1.0, places=9)

    def test_hz_to_inverse_second(self):
        """Test Hz to s^-1 conversion: 1 Hz = 1 s^-1."""
        result = convert_value(1, "Hz", "s^-1")
        self.assertAlmostEqual(result, 1.0, places=9)

    def test_inverse_second_to_hz(self):
        """Test s^-1 to Hz conversion: 1 s^-1 = 1 Hz."""
        result = convert_value(1, "s^-1", "Hz")
        self.assertAlmostEqual(result, 1.0, places=9)

    def test_rad_per_s_to_inverse_second(self):
        """Test rad/s to s^-1 conversion: 1 rad/s = 1/(2*pi) s^-1."""
        result = convert_value(1, "rad/s", "s^-1")
        self.assertAlmostEqual(result, 1 / (2 * math.pi), places=9)

    def test_inverse_second_to_rad_per_s(self):
        """Test s^-1 to rad/s conversion: 1 s^-1 = 2*pi rad/s."""
        result = convert_value(1, "s^-1", "rad/s")
        self.assertAlmostEqual(result, 2 * math.pi, places=9)

    def test_hz_pint_direct_conversion(self):
        """Test Pint direct unit operations for Hz."""
        hz_qty = 1 * ureg.Hz
        rad_per_s = hz_qty.to(ureg.rad / ureg.s)
        self.assertAlmostEqual(rad_per_s.magnitude, 2 * math.pi, places=9)

    def test_hz_uniunit_conversion(self):
        """Test uniUnit Hz conversion with second -> millisecond."""
        u = uniUnit({"second": "millisecond"})
        result = u.to_unit(1 * ureg.Hz)
        self.assertIn("millisecond", str(result.units))
        self.assertAlmostEqual(result.magnitude, 2 * math.pi / 1000, places=6)

    def test_multiple_hz_values(self):
        """Test multiple Hz values conversion."""
        result = convert_value(10, "Hz", "rad/s")
        self.assertAlmostEqual(result, 10 * 2 * math.pi, places=9)

    def test_hz_to_rpm(self):
        """Test Hz to rpm conversion: 1 Hz = 60 rpm."""
        result = convert_value(1, "Hz", "rpm")
        self.assertAlmostEqual(result, 60.0, places=9)

    def test_rpm_to_hz(self):
        """Test rpm to Hz conversion: 60 rpm = 1 Hz."""
        result = convert_value(60, "rpm", "Hz")
        self.assertAlmostEqual(result, 1.0, places=9)

    def test_alternative_inverse_notation(self):
        """Test alternative notations for inverse second."""
        result1 = convert_value(1, "Hz", "1/s")
        result2 = convert_value(1, "Hz", "/s")
        self.assertAlmostEqual(result1, 1.0, places=9)
        self.assertAlmostEqual(result2, 1.0, places=9)
